...
public class JdbcOAuth2AuthorizationQueryService implements OAuth2AuthorizationQueryService {
    ...
    private final String LIST_AUTHORIZED_CLIENTS =
            "select rc.* from oauth2_authorization_consent c " +
                    "inner join oauth2_registered_client rc on rc.id = c.registered_client_id " +
                    "where c.principal_name = ?";

    private final String LIST_AUTHORIZATIONS_QUERY = "" +
    ...
    public JdbcOAuth2AuthorizationQueryService(JdbcOperations operations, RegisteredClientRepository repository) {
    ...
    @Override
    public List<RegisteredClient> listClientsWithConsent(String pricipalName) {
        return this.operations.query(LIST_AUTHORIZED_CLIENTS, registeredClientRowMapper, pricipalName);
    }

    @Override
    public List<OAuth2Authorization> listAuthorizations(String principalName, String clientId) {
        return this.operations.query(LIST_AUTHORIZATIONS_QUERY, oAuth2AuthorizationRowMapper, principalName, clientId);
    }
}
